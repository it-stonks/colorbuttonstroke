...
public class ColorButtonStroke extends SurfaceView implements SurfaceHolder.Callback {
  ...
  /**
   * Радиус закругления внешней стороны границы в пикслеях
   */
  ...
  /**
   * Минимальная скорость движения границы
   */
  private int speedMin = 0;

  /**
   * Максимальная скорость движения границы
   */
  ...
  /**
   * Радиус закругления внутреннего прямоугольника
   */
  private int innerRectRadius;

  /**
   * Радиус закругления внешней части out stroke
   */
  private int outStrokeRadius;

  /**
   * Толщина out stroke
   */
  private int outStrokesThickness;
  ...
  private void init() {
    ...
    setRadius(Utils.dpToPxInt(30, getContext()));
  ...
  public void accelerateStroke(int speed, long duration, boolean continueWithFloatingAcceleration) {
    if (accelerateValueAnimator == null) {
      ...
      accelerateValueAnimator = ValueAnimator.ofInt(speedMin, speedMax);
      ...
      accelerateValueAnimator.addListener(new Animator.AnimatorListener() {
        ...
        public void onAnimationEnd(Animator animation) {
          ...
          if ((int) accelerateValueAnimator.getAnimatedValue() == speedMax) {
            accelerateValueAnimator.setIntValues(speedMax, speedMin);
          } else {
            accelerateValueAnimator.setIntValues(speedMin, speedMax);
          }
      ...
      accelerateValueAnimator.setIntValues((Integer) accelerateValueAnimator.getAnimatedValue(), speedMax);
    }
